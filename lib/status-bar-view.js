...
export default StatusBarView = class StatusBarView {
  constructor() {
  ...
  addLeftTile(options) {
    ...
    const newPriority = (options ? options.priority : undefined) != null ? (options ? options.priority : undefined) : this.leftTiles[this.leftTiles.length - 1].priority + 1;
    let nextItem = null;
  ...
  addRightTile(options) {
    let index;
    const newItem = options.item;
    const newPriority = (options ? options.priority : undefined) != null ? (options ? options.priority : undefined) : this.rightTiles[0].priority + 1;
    let nextItem = null;
  ...
  unsubscribeAllFromBuffer() {
    if (!this.buffer) { return; }
    for (let [event, callback] of this.bufferSubscriptions) {
      this.buffer.off(event, callback);
    }
  }
};
