...
module.exports = {
  activate() {
    ...
    this.subscriptions.add(atom.config.onDidChange('status-bar.fullWidth', () => this.attachStatusBar()))
  ...
  deactivate() {
    if (this.statusBarVisibilitySubscription) this.statusBarVisibilitySubscription.dispose()
    this.statusBarVisibilitySubscription = null

    if (this.gitInfo) this.gitInfo.destroy()
    this.gitInfo = null

    if (this.fileInfo) this.fileInfo.destroy()
    this.fileInfo = null

    if (this.cursorPosition) this.cursorPosition.destroy()
    this.cursorPosition = null

    if (this.selectionCount) this.selectionCount.destroy()
    this.selectionCount = null

    if (this.statusBarPanel) this.statusBarPanel.destroy()
    ...
    if (this.emitters) this.emitters.dispose()
    this.emitters = null
  },
