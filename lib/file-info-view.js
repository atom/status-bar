...
class FileInfoView {
  ...
  update() {
    this.updatePathText()
    this.updateBufferHasModifiedText(this.getActiveItem() && typeof this.getActiveItem().isModified === 'function' && this.getActiveItem().isModified())
  }
  ...
  updatePathText() {
    const activeItem = this.getActiveItem()
    const path = activeItem && typeof activeItem.getPath === 'function' && activeItem.getPath()
    const title = activeItem && typeof activeItem.getTitle === 'function' && activeItem.getTitle()
    if (path) {
      const relativized = atom.project.relativize(path)
